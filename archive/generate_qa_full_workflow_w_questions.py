...
# Function to expand or generate better questions using the LLM
def expand_or_generate_questions(original_question, home_team, incumbent_name):
    prompt = f"Expand or generate a set of questions to better define the original question, considering the home team ({home_team}) and the incumbent ({incumbent_name}). Original question: {original_question}"
    expanded_questions_response = get_ollama_response(prompt, model=MODEL)
    expanded_questions = expanded_questions_response.split('\n')
...
for i, question in enumerate(questions):
    ...
    # Expand or generate better questions
    expanded_questions = expand_or_generate_questions(actual_question, HOME_TEAM, INCUMBENT_NAME)
    all_question_info = []
    all_question_links = []
    ...
    for expanded_question in expanded_questions:
        if tag == "incumbent":
            ...
            question_queries_prompt = f"Generate search queries for the following question relative to the incumbent ({INCUMBENT_NAME}). Ensure to include the incumbent ({INCUMBENT_NAME}) and home team ({HOME_TEAM}): {expanded_question}"
        elif tag == "process":
            ...
            question_queries_prompt = f"Generate search queries for the following question related to process. Ensure to include the incumbent ({INCUMBENT_NAME}) and home team ({HOME_TEAM}): {expanded_question}"
        else:
            print("Tag not identified. Defaulting to general search queries...")
            # Generate default queries for general questions
            question_queries_prompt = f"Generate search queries for the following question. Ensure to include the incumbent ({INCUMBENT_NAME}) and home team ({HOME_TEAM}): {expanded_question}"
        ...
        all_question_info.extend(question_info)
        all_question_links.extend(question_links)

    web_info = initial_info + all_question_info
    web_links = initial_links + all_question_links
